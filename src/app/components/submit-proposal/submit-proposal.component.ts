...
import { Compaign, SubmitProposalModel,
  CampaignQuestion, 
  CampaignAnswer} from 'src/app/models/Compaign';
...
export class SubmitProposalComponent implements OnInit {
  ...
  constructor(private service: CompaignService, private formBuilder: FormBuilder, private toastr: ToastrService, private spinner: NgxSpinnerService, private route: ActivatedRoute, private router: Router) {
  ...
  ngOnInit(): void {
    ...
    debugger
    this._compaign = JSON.parse(localStorage.getItem("campaignList") || "[]");
    this._submitProposalModel.CompaignID = this._compaign.id;
    this._submitProposalModel.clientID = this._compaign.userid.toString();
  ...
  private _getAnswers(answers: CampaignAnswer[]) {
    this._compaign.questions.forEach((x: any) => {
  ...
  SubmitProposal() {
    ...
    if (this._compaign.questions && this._compaign.questions.length > 0) {
    ...
    this._submitProposalModel.clientID = this._submitProposalModel.clientID.toString();
    this._submitProposalModel.Answers = answers;
    this._submitProposalModel.uploadFile = this.ImageBaseData == null ? '' : this.ImageBaseData.toString();
    this.spinner.show();
    this.service.SubmitProposal(this._submitProposalModel).subscribe(
      (res) => {
        this.spinner.hide();
